...
void get_enemy_tunnels(struct cell** map, struct bot* Saken, int y, int x, int deep, int h, int w)
{
    ...
    if (box_count == 2)
    {
        ...
        if (y - 1 >= 0)
        {
            if (map[y - 1][x].path != 1000 && (map[y - 1][x].enemy_tunnel == 0 || map[y - 1][x].enemy_tunnel > 100)) get_enemy_tunnels(map, Saken, y - 1, x, deep + 1, h, w);
        }
        if (y + 1 < h)
        {
            if (map[y + 1][x].path != 1000 && (map[y + 1][x].enemy_tunnel == 0 || map[y + 1][x].enemy_tunnel > 100)) get_enemy_tunnels(map, Saken, y + 1, x, deep + 1, h, w);
        ...
        if (x - 1 >= 0)
        {
            if (map[y][x - 1].path != 1000 && (map[y][x - 1].enemy_tunnel == 0 || map[y][x - 1].enemy_tunnel > 100)) get_enemy_tunnels(map, Saken, y, x - 1, deep + 1, h, w);
        }
        if (x + 1 < w)
        {
            if (map[y][x + 1].path != 1000 && (map[y][x + 1].enemy_tunnel == 0 || map[y][x + 1].enemy_tunnel > 100)) get_enemy_tunnels(map, Saken, y, x + 1, deep + 1, h, w);
