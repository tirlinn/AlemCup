...
int kill_bot(struct cell** map, struct bot* Deidara, struct bot* Saken, struct situation* status, struct pos* aim, struct pos* enemy_aim, int h, int w);
...
int main(void)
{
    ...
    int status->assassin = 1;
    int status->trapped = 0;
    while (1)
    {
        ...
        if (Saken->x != -1)
        {
            ...
            kill_command = kill_bot(map, Deidara, Saken, kill_aim, enemy_aim, h, w, status->assassin);
        }
        ...
        if (map[tmp_aim->y][tmp_aim->x].path <= map[enemy_aim->y][enemy_aim->x].enemy_path)
        {
            ...
            if (extra_steps > 0)
            {
                ...
                if (map[tmp_aim2->y][tmp_aim2->x].path <= map[enemy_aim->y][enemy_aim->x].enemy_path)
                {
                    ...
                    fprintf(stderr, "Found a case to kill for 7 points and we have extra %d steps.\n", extra_steps);
                }
                free(tmp_aim2);
            }
            else
