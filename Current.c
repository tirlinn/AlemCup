...
void out_of_p_sector(struct cell** map, struct bot* Deidara, struct situation* status, int h, int w)
{
    ...
    {
        case 0:
            if (Deidara->x > w / 2 || Deidara->y > h / 2) status->out_of_p_sector = 1;
            break;
        case 1:
            if (Deidara->x < w / 2 || Deidara->y > h / 2) status->out_of_p_sector = 1;
            break;
...
void find_dir(struct cell** map, struct bot* Deidara, struct bot* Saken, struct pos* aim, int h, int w, int* dir, int current_number, struct situation* status, struct pos* enemy_aim)
{
    ...
    */
    ...
    else if ((map[aim->y][aim->x].path >= 5 && map[aim->y][aim->x].path > Deidara->f_r) && map[Deidara->y][Deidara->x].goal >= value && Deidara->f_a > 0 && status->super == 0)
    {
        ...
        *dir = 4;
        ...
        map[Deidara->y][Deidara->x].goal >= value * 2 && Deidara->f_a > 0 && status->super == 0)
    {
        ...
        *dir = 4;
        ...
        map[Deidara->y][Deidara->x].goal >= value && Deidara->a_a > 1 && Deidara->f_a > 0 && status->super == 0)
    {
        ...
        *dir = 4;
        ...
        Deidara->f_r == 2  && map[Deidara->y][Deidara->x].box == -4 && map[Deidara->y][Deidara->x].goal >= value && Deidara->f_a > 0 && status->super == 0)
    {
        *dir = 4;
        fprintf(stderr, "Bomb plant case 5 %d %d %d %d\n", aim->y, aim->x, Deidara->f_a, Deidara->f_r);
    }
    else if (map[aim->y][aim->x].path >= 5 && map[aim->y][aim->x].path > Deidara->f_r && map[Deidara->y][Deidara->x].extragoal > 0 && Deidara->f_a > 0 && map[Deidara->y][Deidara->x].bomb_tick == 0 && status->super == 0)
    {
        ...
        *dir = 4;
        ...
        map[Deidara->y][Deidara->x].extragoal > 0 && map[Deidara->y][Deidara->x].bomb_tick == 0 && Deidara->f_a > 0 && status->super == 0)
    {
        fprintf(stderr, "Bomb plant case 7 %d %d %d %d\n", aim->y, aim->x, Deidara->f_a, Deidara->f_r);
        *dir = 4;
        return;
    }
    else if (Deidara->f_a > 0 && Deidara->y == enemy_aim->y && Deidara->x == enemy_aim->x && map[Deidara->y][Deidara->x].bomb_tick == 0 && map[Deidara->y][Deidara->x].box == 1 && status->super == 0)
    {
